...
import { ProductionData } from '../Pages/Work/CaseStudiesItems/Production'; 
import a09 from "../Assets/Images/09.webp"

const ProductionCaseStudyNav = ({ visitLink }) => {
    const location = useLocation();
    const [navActive, setNavActive] = useState(false);
    
    const currentIndex = ProductionData.findIndex(item => item.link === location.pathname);
    ...
    useEffect(() => {
        const handleScroll = () => {
            ...
            if (footer) {
                ...
                if (scrollValue > 5 && footerPosition > windowHeight) {
                    setNavActive(true);
                } else {
                    setNavActive(true);
                }
    ...
    const prevItem = ProductionData[currentIndex === 0 ? ProductionData.length - 1 : currentIndex - 1];
    const nextItem = ProductionData[currentIndex === ProductionData.length - 1 ? 0 : currentIndex + 1];
    ...
    return (
        <div className={`navItems ${navActive ? 'navActive' : ''}`}>
            <div className='navItemsList'>
                ...
                <div className='navItemsListLinkAll'><Link to="https://www.agency09.in/work/case-studies#Production">All</Link></div>
            </div>
        </div>
    );
};

export default ProductionCaseStudyNav;